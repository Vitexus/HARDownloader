...
class Har extends \Ease\Brick
{
    ...
    public $tmpDir           = '/tmp/';
    public $doneDir          = '/media/vitex/data2/img';
    ...
    {
        return $this->doCurlRequest("https://www.himalayanart.org/items/$itemId/images/primary")
            == 200;
    }
    ...
    public function obtain($itemId)
    {
        if ($this->loadImagePage($itemId)) {
            ...
            if (preg_match_all('/https?\:\/\/[^\" ]+metadata\.json/i',
                    $this->lastCurlResponse, $urls)) {
                file_put_contents(str_replace('.png', '.html',
                        $this->targetImage($itemId)), $this->lastCurlResponse);
                if (file_exists($this->targetImage($itemId))) {
                    $this->addStatusMessage($this->targetImage($itemId).' already exists',
                        'info');
                } else {
                    $this->tileDone = 0;
                    $metaUrl        = $urls[0][0];
                    $baseUrl        = str_replace(basename($metaUrl), '',
                        $metaUrl);
                    $metaData       = json_decode(file_get_contents($metaUrl),
                        true);
                    ...
                    $this->addStatusMessage('ItemID: '.$itemId.' saved as  '.'img/'.$itemId.'.png',
                        'success');
                    return true;
                }
            }
